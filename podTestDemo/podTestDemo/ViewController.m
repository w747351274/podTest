...
CG_INLINE Class funcTest(NSString *str){
    ...
    if([str isEqualToString:@"001-func1"] ) {
        return [PodTestViewController class];
    }else if ([str isEqualToString:@"001-func2"]){
        return [PayViewController class];
    }else if ([str isEqualToString:@"001-func3"]){
        return [MyFuncViewController class];
    }else if ([str isEqualToString:@"001-func4"]){
        return [Myfunc2ViewController class];
    }else if ([str isEqualToString:@"web"]){
        return [UIWebViewController class];
    }else{
        return Nil;
    }
...
-(void)setFuncDataSource{
    ...
    [propertyDic[@"funcconfigList"] enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        if (showFlag == 1) {
            [funcconfigList addObject:[[SettingItem alloc]initWithIcon:obj[@"iconurl"] withName:obj[@"funname"] withClass:cls withFuncUrl:obj[@"funcurl"]]];
        }
    }];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (item.cls ==[UIWebViewController class]) {
        UIWebViewController *web = (UIWebViewController*)controller;
        [web setUrlString:item.funcURL];
    }
